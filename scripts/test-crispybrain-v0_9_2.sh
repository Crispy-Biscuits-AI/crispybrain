...
SEED_SPEC_PATH="${REPO_ROOT}/seed-data/crispybrain-v0_9_2-eval-seed.json"
ASSISTANT_CONTAINER_PATH="/tmp/crispybrain-assistant-v0_9_2-test.json"
DEMO_CONTAINER_PATH="/tmp/crispybrain-demo-v0_9_2-test.json"
INGEST_CONTAINER_PATH="/tmp/crispybrain-ingest-v0_9_2-test.json"
...
const response = JSON.parse(process.env.RESPONSE_JSON);
const answer = typeof response.answer === 'string' ? response.answer : '';
const claimSupportCounts = Array.isArray(response.claim_support_counts) ? response.claim_support_counts : [];
...
const ok = response.ok === true
  ...
  && answer.includes('Support: 2')
  && answer.includes('Support: 1')
  && answer.includes('Hint: Most supported claim is amber-9')
  && answer.includes('Hint: Most recent claim is cobalt-3');
process.exit(ok ? 0 : 1);
EOF
}
...
run_case() {
  ...
  measure_post_json 'http://localhost:5678/webhook/assistant' "$(jq -cn \
    --arg query "${query}" \
    --arg correlation_id "corr-v092-${case_id}" \
    --arg session_id "cb-v092-${case_id}" \
    --arg project_slug "${PROJECT_SLUG}" \
...
RUN_TOKEN="cbv092$(date +%s)${RANDOM}"
PROJECT_SLUG="${CRISPYBRAIN_EVAL_PROJECT_SLUG:-alpha-v092-${RUN_TOKEN}}"
...
seed_note() {
  ...
  ingest_runtime_note "${filepath}" "${filename}" "${content}" "corr-v092-${key}-ingest"
  row_id="$(latest_memory_id_for_filepath "${filepath}")"
  assert_nonempty_string "${row_id}" "${key} row id"
  if [[ "${reviewed}" == "true" ]]; then
    mark_reviewed "${row_id}" "v0.9.2 evaluation note ${key}"
  fi
...
run_case 'eval-07' 'weighted conflict hints' 'What is the harbor relay protocol?' 'weighted conflicts expose support counts, most_supported_claim, and most_recent_claim without leaving conflict mode' evaluate_weighted_conflict
...
crispybrain_harness_log "SUMMARY: next_debug_step=curl -sS -H 'Content-Type: application/json' -d '{\"message\":\"What is the harbor relay protocol?\",\"project_slug\":\"${PROJECT_SLUG}\",\"top_k\":8}' http://localhost:5678/webhook/assistant | jq '{answer_mode,conflict_flag,conflict_severity,claim_support_counts,most_supported_claim,most_recent_claim,entity_focus,filtered_candidate_count,conflict_details,retrieval,grounding,sources,retrieved_candidates}'"

if (( failed_count > 0 )); then
  crispybrain_harness_fail "v0.9.2 evaluation pack failed case(s): ${failed_cases[*]}"
fi

crispybrain_harness_pass "CrispyBrain v0.9.2 verified the 7-case retrieval, trimming, and conflict-hints pack"
