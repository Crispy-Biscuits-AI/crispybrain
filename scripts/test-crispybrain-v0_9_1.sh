...
SEED_SPEC_PATH="${REPO_ROOT}/seed-data/crispybrain-v0_9_1-eval-seed.json"
ASSISTANT_CONTAINER_PATH="/tmp/crispybrain-assistant-v0_9_1-test.json"
DEMO_CONTAINER_PATH="/tmp/crispybrain-demo-v0_9_1-test.json"
INGEST_CONTAINER_PATH="/tmp/crispybrain-ingest-v0_9_1-test.json"
...
run_case() {
  ...
  measure_post_json 'http://localhost:5678/webhook/assistant' "$(jq -cn \
    --arg query "${query}" \
    --arg correlation_id "corr-v091-${case_id}" \
    --arg session_id "cb-v091-${case_id}" \
    --arg project_slug "${PROJECT_SLUG}" \
...
RUN_TOKEN="cbv091$(date +%s)${RANDOM}"
PROJECT_SLUG="${CRISPYBRAIN_EVAL_PROJECT_SLUG:-alpha-v091-${RUN_TOKEN}}"
...
seed_note() {
  ...
  ingest_runtime_note "${filepath}" "${filename}" "${content}" "corr-v091-${key}-ingest"
  row_id="$(latest_memory_id_for_filepath "${filepath}")"
  assert_nonempty_string "${row_id}" "${key} row id"
  if [[ "${reviewed}" == "true" ]]; then
    mark_reviewed "${row_id}" "v0.9.1 evaluation note ${key}"
  fi
...
crispybrain_harness_log "SUMMARY: next_debug_step=curl -sS -H 'Content-Type: application/json' -d '{\"message\":\"What is the beacon protocol?\",\"project_slug\":\"${PROJECT_SLUG}\",\"top_k\":8}' http://localhost:5678/webhook/assistant | jq '{answer_mode,conflict_flag,conflict_severity,entity_focus,filtered_candidate_count,conflict_details,retrieval,grounding,sources,retrieved_candidates}'"

if (( failed_count > 0 )); then
  crispybrain_harness_fail "v0.9 evaluation pack failed case(s): ${failed_cases[*]}"
fi

crispybrain_harness_pass "CrispyBrain v0.9.1 verified the 6-case retrieval, trimming, and conflict pack"
