...
[[ -f "${SQL_PATH}" ]] || crispybrain_harness_fail "SQL file not found: ${SQL_PATH}"
[[ -d "${WORKFLOW_DIR}" ]] || crispybrain_harness_fail "Workflow directory not found: ${WORKFLOW_DIR}"

crispybrain_harness_apply_sql_file "${SQL_PATH}" "/tmp/crispybrain-v0_4-upgrade.sql" "crispybrain-v0.4 SQL migration"

TARGET_FOLDER_ID="$(crispybrain_harness_ensure_folder "${TARGET_FOLDER_NAME}")"
TARGET_FOLDER_ID="$(printf '%s' "${TARGET_FOLDER_ID}" | tr -d '[:space:]')"
[[ -n "${TARGET_FOLDER_ID}" ]] || crispybrain_harness_fail "Could not ensure folder ${TARGET_FOLDER_NAME}"
crispybrain_harness_log "Using folder ${TARGET_FOLDER_NAME} (${TARGET_FOLDER_ID})"
...
for workflow_path in "${WORKFLOW_DIR}"/*.json; do
  ...
  if ! jq -e '.id and .name and .nodes' "${workflow_path}" >/dev/null 2>&1; then
    crispybrain_harness_log "Skipping non-workflow JSON artifact ${workflow_path}"
    continue
  fi

  workflow_id="$(jq -r '.id' "${workflow_path}")"
  workflow_name="$(jq -r '.name' "${workflow_path}")"
  [[ -n "${workflow_id}" && "${workflow_id}" != "null" ]] || crispybrain_harness_fail "Workflow id missing in ${workflow_path}"
  [[ "${workflow_name}" == "${workflow_id}" ]] || crispybrain_harness_fail "Workflow ${workflow_path} has mismatched name/id: ${workflow_name} vs ${workflow_id}"

  rendered_path="${TEMP_RENDER_DIR}/${workflow_id}.json"
  container_path="/tmp/${workflow_id}.json"
  crispybrain_harness_render_workflow_for_folder "${workflow_path}" "${rendered_path}" "${TARGET_FOLDER_ID}" "${TARGET_FOLDER_NAME}"
  crispybrain_harness_copy_workflow "${rendered_path}" "${container_path}" "${workflow_name}"
  crispybrain_harness_import_workflow "${container_path}" "${workflow_name}"
  imported_workflow_ids+=("${workflow_id}")
done

(( ${#imported_workflow_ids[@]} > 0 )) || crispybrain_harness_fail "No importable workflow JSON files were found in ${WORKFLOW_DIR}"

crispybrain_harness_log "Listing workflows to confirm imports"
WORKFLOW_LIST_OUTPUT="$(crispybrain_harness_list_workflows)"
crispybrain_harness_log "${WORKFLOW_LIST_OUTPUT}"

for workflow_id in "${imported_workflow_ids[@]}"; do
  crispybrain_harness_assert_workflow_visible "${workflow_id}" "${WORKFLOW_LIST_OUTPUT}"
  crispybrain_harness_assert_workflow_in_folder "${workflow_id}" "${TARGET_FOLDER_ID}"
done

crispybrain_harness_assert_folder_has_no_legacy_prefixes "${TARGET_FOLDER_ID}"

crispybrain_harness_log "Minting local n8n auth cookie"
AUTH_COOKIE="$(crispybrain_harness_mint_auth_cookie)"
[[ -n "${AUTH_COOKIE}" ]] || crispybrain_harness_fail "Could not mint a local n8n auth cookie"
...
[[ "${CRISPYBRAIN_HARNESS_LAST_HTTP_STATUS}" == "200" ]] || crispybrain_harness_fail "Could not fetch workflow details for ${ASSISTANT_WORKFLOW_ID}"
