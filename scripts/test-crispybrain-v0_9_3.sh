...
SEED_SPEC_PATH="${REPO_ROOT}/seed-data/crispybrain-v0_9_3-eval-seed.json"
ASSISTANT_CONTAINER_PATH="/tmp/crispybrain-assistant-v0_9_3-test.json"
DEMO_CONTAINER_PATH="/tmp/crispybrain-demo-v0_9_3-test.json"
INGEST_CONTAINER_PATH="/tmp/crispybrain-ingest-v0_9_3-test.json"
...
const ok = response.ok === true
  ...
  && answer.includes('Support: 1 (deduped: 1)')
  && typeof response.conflict_summary_hint === 'string'
  && response.conflict_summary_hint.includes('support is tied')
  && !candidates.some((candidate) => typeof candidate?.title === 'string' && candidate.title.includes(process.env.NOISE_TITLE));
process.exit(ok ? 0 : 1);
EOF
}
...
const ok = response.ok === true
  ...
  && typeof response.conflict_summary_hint === 'string'
  && response.conflict_summary_hint.includes('amber-9 has stronger support')
  && response.conflict_summary_hint.includes('cobalt-3 is more recent')
  && answer.includes('Dominant: dominant')
  && answer.includes('Confidence: medium')
  && answer.includes('Support: 2 (deduped: 2)')
  && answer.includes('Support: 1 (deduped: 1)')
  && answer.includes('Hint: amber-9 has stronger support (2 vs 1), but cobalt-3 is more recent.');
process.exit(ok ? 0 : 1);
EOF
}
...
const ok = response.ok === true
  ...
  && response.conflict_summary_hint.includes('support is tied')
  && typeof response.answer === 'string'
  && response.answer.includes('Support: 2 (deduped: 1)');
process.exit(ok ? 0 : 1);
EOF
}
...
run_case() {
  ...
  measure_post_json 'http://localhost:5678/webhook/assistant' "$(jq -cn \
    --arg query "${query}" \
    --arg correlation_id "corr-v093-${case_id}" \
    --arg session_id "cb-v093-${case_id}" \
    --arg project_slug "${PROJECT_SLUG}" \
...
RUN_TOKEN="cbv093$(date +%s)${RANDOM}"
PROJECT_SLUG="${CRISPYBRAIN_EVAL_PROJECT_SLUG:-alpha-v093-${RUN_TOKEN}}"
...
seed_note() {
  ...
  ingest_runtime_note "${filepath}" "${filename}" "${content}" "corr-v093-${key}-ingest"
  row_id="$(latest_memory_id_for_filepath "${filepath}")"
  assert_nonempty_string "${row_id}" "${key} row id"
  if [[ "${reviewed}" == "true" ]]; then
    mark_reviewed "${row_id}" "v0.9.3 evaluation note ${key}"
  fi
...
run_case 'eval-07' 'dominant conflict with recency disagreement' 'What is the harbor relay protocol?' 'dominant support stays conflict-only and summary hint notes the more recent weaker claim' evaluate_weighted_conflict
run_case 'eval-08' 'tie conflict' 'What is the lantern mesh protocol?' 'tie conflicts expose dominant_claim_status=tie and low confidence' evaluate_tie_conflict
run_case 'eval-09' 'duplicate support suppression' 'What is the north gate protocol?' 'duplicate notes do not inflate deduped support counts' evaluate_duplicate_conflict
...
crispybrain_harness_log "SUMMARY: next_debug_step=curl -sS -H 'Content-Type: application/json' -d '{\"message\":\"What is the north gate protocol?\",\"project_slug\":\"${PROJECT_SLUG}\",\"top_k\":8}' http://localhost:5678/webhook/assistant | jq '{answer_mode,conflict_flag,conflict_severity,dominant_claim_status,claim_confidence,conflict_summary_hint,claim_support_counts_raw,claim_support_counts_deduped,most_supported_claim,most_recent_claim,entity_focus,filtered_candidate_count,conflict_details,retrieval,grounding,sources,retrieved_candidates}'"

if (( failed_count > 0 )); then
  crispybrain_harness_fail "v0.9.3 evaluation pack failed case(s): ${failed_cases[*]}"
fi

crispybrain_harness_pass "CrispyBrain v0.9.3 verified the 9-case retrieval, tie-handling, and confidence-surface pack"
