...
SEED_SPEC_PATH="${REPO_ROOT}/seed-data/crispybrain-v0_9_4-eval-seed.json"
ASSISTANT_CONTAINER_PATH="/tmp/crispybrain-assistant-v0_9_4-test.json"
DEMO_CONTAINER_PATH="/tmp/crispybrain-demo-v0_9_4-test.json"
INGEST_CONTAINER_PATH="/tmp/crispybrain-ingest-v0_9_4-test.json"
...
const ok = response.ok === true
  ...
  && answer.includes('Support: 1 | Deduped: 1 | Weighted: 2.0')
  && typeof response.conflict_summary_hint === 'string'
  && response.conflict_summary_hint.includes('weighted support and deduplicated support are both tied')
  && !candidates.some((candidate) => typeof candidate?.title === 'string' && candidate.title.includes(process.env.NOISE_TITLE));
process.exit(ok ? 0 : 1);
EOF
}

evaluate_quality_weighted_conflict() {
  local response_json="$1"
  RESPONSE_JSON="${response_json}" node - <<'EOF'
const response = JSON.parse(process.env.RESPONSE_JSON);
const answer = typeof response.answer === 'string' ? response.answer : '';
...
const ok = response.ok === true
  ...
  && response.dominant_claim_basis === 'weighted_support'
  && response.claim_confidence === 'medium'
  && amberDeduped?.count === 1
  && cobaltDeduped?.count === 1
  && amberWeighted?.weighted_support === 2
  && cobaltWeighted?.weighted_support === 1
  && response.most_supported_claim?.value === 'amber-9'
  && response.most_supported_claim?.basis === 'weighted_support'
  && Array.isArray(response.selected_sources)
  && response.selected_sources.some((source) => source?.source_quality === 'high')
  && response.selected_sources.some((source) => source?.source_quality === 'low')
  && typeof response.conflict_summary_hint === 'string'
  && response.conflict_summary_hint.includes('stronger weighted support despite equal deduplicated support')
  && answer.includes('Basis: weighted_support')
  && answer.includes('Support: 1 | Deduped: 1 | Weighted: 2.0')
  && answer.includes('Support: 1 | Deduped: 1 | Weighted: 1.0');
process.exit(ok ? 0 : 1);
EOF
}

evaluate_weighted_conflict() {
  local response_json="$1"
  RESPONSE_JSON="${response_json}" node - <<'EOF'
const response = JSON.parse(process.env.RESPONSE_JSON);
const answer = typeof response.answer === 'string' ? response.answer : '';
...
const ok = response.ok === true
  ...
  && typeof response.conflict_summary_hint === 'string'
  && response.conflict_summary_hint.includes('amber-9 has stronger weighted support')
  ...
  && answer.includes('Support: 2 | Deduped: 2 | Weighted: 4.0')
  && answer.includes('Support: 1 | Deduped: 1 | Weighted: 2.0')
  && answer.includes('Hint: amber-9 has stronger weighted support (4.0 vs 2.0). But cobalt-3 is more recent.');
process.exit(ok ? 0 : 1);
EOF
}
...
const ok = response.ok === true
  ...
  && amberDeduped?.count === 1
  && cobaltDeduped?.count === 1
  && amberWeighted?.weighted_support === 1
  && cobaltWeighted?.weighted_support === 2
  && response.most_supported_claim?.value === 'cobalt-3'
  && typeof response.conflict_summary_hint === 'string'
  && response.conflict_summary_hint.includes('stronger weighted support despite equal deduplicated support')
  && typeof response.answer === 'string'
  && response.answer.includes('Basis: weighted_support')
  && response.answer.includes('Support: 2 | Deduped: 1 | Weighted: 1.0')
  && response.answer.includes('Support: 1 | Deduped: 1 | Weighted: 2.0');
process.exit(ok ? 0 : 1);
EOF
}
...
run_case() {
  ...
  measure_post_json 'http://localhost:5678/webhook/assistant' "$(jq -cn \
    --arg query "${query}" \
    --arg correlation_id "corr-v094-${case_id}" \
    --arg session_id "cb-v094-${case_id}" \
    --arg project_slug "${PROJECT_SLUG}" \
...
RUN_TOKEN="cbv094$(date +%s)${RANDOM}"
PROJECT_SLUG="${CRISPYBRAIN_EVAL_PROJECT_SLUG:-alpha-v094-${RUN_TOKEN}}"
...
seed_note() {
  ...
  ingest_runtime_note "${filepath}" "${filename}" "${content}" "corr-v094-${key}-ingest"
  row_id="$(latest_memory_id_for_filepath "${filepath}")"
  assert_nonempty_string "${row_id}" "${key} row id"
  if [[ "${reviewed}" == "true" ]]; then
    mark_reviewed "${row_id}" "v0.9.4 evaluation note ${key}"
  fi
...
seed_note quality_conflict_high_a
seed_note quality_conflict_low_a
...
run_case 'eval-07' 'equal deduped support, weighted dominance' 'What is the quay switch protocol?' 'equal deduplicated support can still resolve to weighted dominance when one side is backed by higher-quality notes' evaluate_quality_weighted_conflict
run_case 'eval-08' 'dominant conflict with recency disagreement' 'What is the harbor relay protocol?' 'weighted support stays conflict-only and summary hint notes the more recent weaker claim' evaluate_weighted_conflict
run_case 'eval-09' 'tie conflict' 'What is the lantern mesh protocol?' 'weighted ties expose dominant_claim_status=tie with low confidence' evaluate_tie_conflict
run_case 'eval-10' 'duplicate-heavy weighted conflict' 'What is the north gate protocol?' 'duplicate-heavy evidence loses its raw-count advantage once dedupe and source quality are applied' evaluate_duplicate_conflict
...
crispybrain_harness_log "SUMMARY: likely_failure_point=assistant candidate trimming, entity focus scoring, lexical fallback merge, or conflict severity routing"
crispybrain_harness_log "SUMMARY: next_debug_step=curl -sS -H 'Content-Type: application/json' -d '{\"message\":\"What is the north gate protocol?\",\"project_slug\":\"${PROJECT_SLUG}\",\"top_k\":8}' http://localhost:5678/webhook/assistant | jq '{answer_mode,conflict_flag,conflict_severity,dominant_claim_status,dominant_claim_basis,claim_confidence,conflict_summary_hint,claim_support_counts_raw,claim_support_counts_deduped,claim_weighted_support,most_supported_claim,most_recent_claim,source_quality_breakdown,entity_focus,filtered_candidate_count,conflict_details,retrieval,grounding,sources,retrieved_candidates}'"

if (( failed_count > 0 )); then
  crispybrain_harness_fail "v0.9.4 evaluation pack failed case(s): ${failed_cases[*]}"
fi

crispybrain_harness_pass "CrispyBrain v0.9.4 verified the 10-case weighted-support and source-quality pack"
