...
`v0.9.5` does not redesign retrieval further.
Its main follow-on work is still in the answer layer: the preserved conflicting candidates now carry enough grouped source support, duplicate-aware support identities, source-quality labels, source-independence labels, evidence clusters, and recency metadata for the trust surface to expose better hints.
...
In `v0.9.5`, that preserved evidence is also used to compute duplicate-aware support counts, source-quality labels, weighted support, evidence clusters, source independence, and independence-adjusted dominance handling without changing retrieval itself.
